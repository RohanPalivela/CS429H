...
int main() {
	...
	while (time < 13) {
		...
		for (int i = 0; i < num_people_in; i++) {
			...
			// ll_node new_node = {{0, rand() % 150}, NULL, NULL};
		...
		for (int i = 0; i < 8; i++) {
			...
			if (cur->stopped) {
				...
				if (cur->time_waiting == 10) {
					printf("elevator %d has waited for 10 seconds and/or is full, moving on\n", cur->id);
					cur->stopped = false;
					continue;
				}
				...
				if (floors[cur->floor]->size > 0 && cur->people_count == 0 && !elevator_here[cur->floor]) {
					printf("stopping to pick up people on floor %d \n", cur->floor);
					cur->stopped = true;
					elevator_here[cur->floor] = true;
				} else if (cur->upwards) { 
					if (cur-> floor == 150) {
						printf("Changing direction because floor is 150 or no riders\n");
						cur->upwards = false;
						continue;
					}
					...
					if (cur->riders[0] != NULL && cur->floor == cur->riders[0]->desired_floor) {
						...
						cur->stopped = true;
					} else if (!keep_going(floors,cur->floor,cur->upwards) && cur->riders[0] == NULL) { // at end 
					...
					if (cur->riders[0] != NULL && cur->floor == cur->riders[cur->people_count - 1]->desired_floor) {	
						printf("stopping to drop off people on floor %d \n", cur->floor);
						printf("\n");
						cur->stopped = true;
					} else if (!keep_going(floors, cur->floor, cur->upwards) || cur->riders[0] == NULL) { // at end or no more riders in elevator
						printf("Changing direction because floor is 0 or no riders\n");
						cur->upwards = true;
					} else {
						cur->distance_traveled++;
						cur->floor--;
